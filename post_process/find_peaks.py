...
def merge_bounding_boxes(bounding_boxes, expand_margin_x=5):
    merged_boxes = []
    for box in bounding_boxes:
        ...
        for index, m_box in enumerate(merged_boxes):
            ...
            if x1 <= mx2 + expand_margin_x and x2 + expand_margin_x >= mx1 and y1 <= my2 and y2 >= my1:
...
def find_box(matrix, expand_margin=1, expand_margin_x=5):
    matrix = matrix.copy()
    matrix[matrix > 0] = 1
    ...
    merged_boxes = merge_bounding_boxes(bounding_boxes, expand_margin_x=expand_margin_x)
