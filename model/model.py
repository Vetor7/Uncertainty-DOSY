    ...
    def __init__(self):
        ...
        if config.Type=='T1T2':
            self.layers = nn.Sequential(
                nn.Conv2d(1, 10, kernel_size=(1, 7), stride=1, padding=(0, 2)),
                LayerNorm(8), nn.ReLU(),
...
class ModifiedTransformerEncoder(nn.Module):
    def __init__(self, num_layers, dim=140):
        ...
        self.fc = nn.Linear(config.signal_dim, dim)
...
class DReaM_net(pl.LightningModule):
    ...
    def __init__(self, lr=config.lr, n_layers=config.n_layers, n_heads=config.n_heads, label_size=config.label_size,
        ...
        self.encoder_var = ModifiedTransformerEncoder(n_layers, self.hparams.label_size).to(DEVICE)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        mean, var = self(val_input)
        loss = beta_nll_loss(mean, var, val_label, beta=0)
        mse_loss = F.mse_loss(mean, val_label)
