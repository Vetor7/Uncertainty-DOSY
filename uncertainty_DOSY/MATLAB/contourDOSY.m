function contourDOSY(Type, Result_LRsPILT, Result_CoMeF, Z, HNMR, b, ppm, ...
    cs, idx_peaks, cs1, cs2, dc1, dc2, t, S, f1,var)
    cs_spec = zeros([length(ppm), 1]);
    if length(var(1,:)) ==1
    var_spec = zeros([length(ppm), 1]);
    var_spec(idx_peaks, :) = var;
    else
    var_spec = zeros([length(Z(1, :)), length(ppm)]);
    var_spec(:, idx_peaks) = var.';
    end
    spec_whole = zeros([length(Z(1, :)), length(ppm)]);
    Signals = zeros([length(ppm), length(b)]);
    Signals(idx_peaks, :) = S;
    ...
    elseif Type == "GSP"
        ...
        xlabel(t, 'Chemical Shift(ppm)');
        ylabel(t, 'Diffusion Coefficient(10^{-10}m^2/s)');
        disp('请用鼠标在等高图上选择一个区域。');
        nexttile(8, [2, 1])
        if length(var(1,:)) ==1
        plot(ppm,var_spec, "Color",'r');set(gca,'Xdir','reverse');
        xlim([cs1,cs2]);
        else
        contour(ppm,decay_range*(0.8/b(end)),var_spec, 10);
        title('Uncertainty');
        colorbar()
        set(gca,'Ydir','reverse','Xdir','reverse'); 
        xlim([cs1,cs2]);
        ...
        for i = 1:10
        figure(f1);
        [x, y] = ginput(1); % 等待用户选择一个点
        [~, x_idx] = min(abs(x - ppm(1, :)));
        ...
        if ~isempty(x_idx)
            ...
            % 绘制一维图
        figure(f2);
        plot(x_values,Signals(x_idx,:)/Signals(x_idx,1));
        hold on
    ...
    elseif Type == "M6"
        ...
        nexttile(5, [3, 1])
        DiffCoef = [3.2, 4.3, 5.0, 6.3, 8.2, 10.5];
        contour(ppm,decay_range*(0.8/b(end)),var_spec,20);
        colorbar();
        title('Uncertainty');
        set(gca,'Ydir','reverse','Xdir','reverse'); 
        xlim([cs1,cs2]);
    ...
    elseif Type == "9100"
        ...
        xlabel(t, 'Chemical Shift(ppm)');
        ylabel(t, 'Diffusion Coefficient(10^{-10}m^2/s)');
        DiffCoef = [10.5, 11.3, 14.3,16.3];
        for i = 1:length(DiffCoef)
            l = line(gca,get(gca,'xlim'),DiffCoef(i)*ones(1,2),'LineWidth',0.8,'color',[0.85 0.85 0.85],'LineStyle','--');
            uistack(l, "bottom");
        end

        text(4, 10, "n-butyric acid", FontSize=4, Color=[1, 0.07, 0.07])
        text(4, 11, "n-butyl alcoho", FontSize=4, Color=[1, 0.07, 0.07])
        text(4, 16, "ethanol", FontSize=4, Color=[1, 0.07, 0.07])
        text(4, 14, "acetic acid", FontSize=4, Color=[1, 0.07, 0.07])
